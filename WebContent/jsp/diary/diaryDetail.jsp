			...
			<div class="container">
				<center>
					...
					<table class="table">
						...
						<tr>
							<th>글쓴이</th>
							<td><input type="text" class="form-control" value="${diary.writer}" readonly></td>
						</tr>
						<tr>
							<th>제목</th>
							<td><input type="text" class="form-control" value="${diary.title}" readonly></td>
						</tr>
						<tr>
							<th>내용</th>
							<td><textarea class="form-control" rows="3" readonly>${diary.content}</textarea></td>
						</tr>
						<tr>
							<th>작성일</th>
							<td><input type="text" class="form-control" value="${diary.updateDate}" readonly></td>
						</tr>
						<tr>
							...
							<c:when test="${ commentNo eq comment.commentNo }">
								<form action="${pageContext.request.contextPath }/jsp/diary/diarycommentprocess?no=${diary.no}" method="post">							
									<tr>
										...
										<td>${comment.writer}</td>
										<td></a><input type="text" value="${comment.content}" name="content"></td>
										<td>${comment.regDate}</td>
				...
				<form action="${pageContext.request.contextPath }/jsp/diary/diarycommentprocess?no=${diary.no}" method="post">
					<table class="table">
						<tbody>
							<tr>
								<th>글쓴이</th>
								<td><input type="text" name="writer" class="form-control" value="${user.id}" readonly></td>
								<th>내용</th>
